...
class ObjectiveFunction:
    ...
    def get_obj_diff(self, idx_selected, idx_deselected):
        ...
        # Eq.(39)
        self.temp[:, :] = 2 * (self.mat_T.transpose() @ self.mat_R) +                           (self.mat_L.transpose() @ self.mat_L) @ (self.mat_H.transpose() @ self.mat_R)
        obj_diff = self.cov_matrix[idx_j, idx_j] - self.cov_matrix[idx_i, idx_i] +                    np.trace(
...
class InstanceNotFittedError(Exception):
    def __init__(self):
        message = "This Feature Selector instance is not fitted yet. "                   "Call 'fit' or 'fit_transform' with your dataset 'X'."
