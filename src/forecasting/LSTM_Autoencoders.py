...
def model_lstm_autoencoders(X,y,dim1,dim2,dim3,dp):
    inputs = Input(shape=(X.shape[1],X.shape[2]), name='input')	
    L1 = LSTM(dim1, activation='relu',return_sequences=True)(inputs)
    L2 = Dropout(dp)(L1)
    L3 = LSTM(dim2, activation='relu',return_sequences=False)(L2)
    L4 = Dropout(dp)(L3)
    L5 = RepeatVector(X.shape[1])(L4)
    L10= LSTM(dim3, activation='relu')(L5)
    L11 = Dropout(dp)(L10)
    output2 = Dense(y.shape[1], name= 'reg')(L11)
    L6 = LSTM(dim2, activation='relu', return_sequences=True)(L5)
    L7 = Dropout(dp)(L6)
    L8 = LSTM(dim1, activation='relu',return_sequences=True)(L7)
    L9 = Dropout(dp)(L8)
    output = TimeDistributed(Dense(X.shape[2], name='time_distributed'))(L9)
      
    model = Model(inputs=inputs,outputs=[output,output2])
    model.compile(loss={'reg': 'mean_squared_error', 
                    'time_distributed': 'mean_squared_error'},
              optimizer='adam',
              metrics={'reg':'mean_squared_error','time_distributed':'mean_squared_error'})
    return model
...
def training(X_train,y_train,X_test, y_test,model,patience,direccion,bs,n_epochs,verbose):
    ...
    es = EarlyStopping(monitor='val_reg_loss', mode='min', verbose=verbose, patience=patience)
    mc = ModelCheckpoint(direccion+'model_lstm_autoencoder.h5', monitor='val_reg_loss', mode='min', verbose=verbose, save_best_only=True)

    model.fit(X_train, {'reg': y_train, 'time_distributed':X_train},batch_size=bs,epochs=n_epochs,verbose=verbose,validation_data=(X_test,{'reg': y_test, 'time_distributed':X_test }),callbacks=[es,mc])
